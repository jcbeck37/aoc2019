function crossedWires() {
  function processInput(input) {
    ...
    // sample2 = sample2 + 'U98,R91,D20,R16,D67,R40,U7,R15,U6,R7';
  ...
  function populateWire(wirePath, oldWire) {
    ...
    directions.map((d, idx) => {
      ...
      if (oldWire !== undefined) {
        ...
        const results = checkIntersections(oldWire, newPath);
        if (results.length > 0) {
          results.map((result) => {
            ...
            const wire1steps = calculateSteps(oldWire, wire1Idx - 1);
            const wire2steps = calculateSteps(wireCoords, idx - 1);
            const final1steps = countSteps(x1, y1, intX, intY);
            const final2steps = countSteps(x, y, intX, intY);
            const totalSteps = wire1steps + wire2steps + final1steps + final2steps;
